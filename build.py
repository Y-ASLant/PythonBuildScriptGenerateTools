...
def main():
    ...
    log_info("输出目录: PythonBuildScriptGenerate")
    ...
    # Nuitka编译参数
    args = [
        "nuitka",    "--standalone",    "--onefile",    "--assume-yes-for-downloads",    "--remove-output",    "--mingw64",    "--quiet",    "--windows-company-name=ASLant",    "--windows-file-version=1.0.0",    "--output-dir=PythonBuildScriptGenerate",    "--output-filename=PythonBuildScriptGenerate",    "--jobs=16",    "--windows-icon-from-ico=app.ico",    "--linux-icon=app.ico",    "--nofollow-import-to=nuitka",    "main.py",
    ]
    ...
    log_success("🎉 构建完成！")
    log_info("输出位置: PythonBuildScriptGenerate")
...
def copy_additional_files():
    ...
    build_output_dir = Path("PythonBuildScriptGenerate")
